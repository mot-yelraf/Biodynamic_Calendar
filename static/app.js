...
async function loadCalendar(monthKey = "", preferredDate = "", options = {}) {
  ...
  try {
    const resp = await fetch(requestMonth ? `/api/calendar?month=${encodeURIComponent(requestMonth)}` : "/api/calendar");
...
async function fetchDailySummary(dayIso) {
  ...
  const request = fetch(`/api/daily-summary?day=${encodeURIComponent(dayIso)}`)
...
async function loadCalendarRange(monthKey, options = {}) {
  ...
  setRangeBusy(true);
  try {
    const resp = await fetch(`/api/calendar-range?start=${encodeURIComponent(requestedMonth)}&months=13`);
    const payload = await resp.json();
...
document.getElementById("plantingForm").addEventListener("submit", async (ev) => {
  ...
  try {
    const resp = await fetch("/api/planting", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    });
...
document.getElementById("plantingList").addEventListener("click", async (ev) => {
  ...
  try {
    const resp = await fetch(`/api/planting/${encodeURIComponent(id)}`, { method: "DELETE" });
    const payload = await resp.json();
    if (!resp.ok || !payload.ok) {
